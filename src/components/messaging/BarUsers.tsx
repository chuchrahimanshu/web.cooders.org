  ...
  return (
    <div className='h-full pt-[6px]'>
      {users && users.length > 0 && users.map((user) => (
        <div key={user.id} className='flex justify-between px-3 py-[10px] hover:bg-base-border transition-all duration-300 ease-in-out cursor-pointer'>
          <div className='flex gap-2 items-center w-[80%]'>
            <img src={user.image} alt="User" className='h-11 w-11 rounded-full object-cover' />
          ...
          <div className='flex flex-col justify-start pt-[2px] gap-[6px]'>
            <p className='text-[10px] font-medium opacity-75 self-end'>{user.messageTime}</p>
            <div className='self-end bg-action-button h-5 w-5 flex items-center justify-center rounded-full opacity-70'>
              <p className='text-[14px] font-medium -ml-[1px] text-white'>1</p>
            </div>
          </div>
        </div>
      ))}
